...
/**
 * Loops fire-door move/alarm while opening/closing and stops exactly when the anim ends
 * (matches 1.7.10 AudioWrapper loop start/stop — {@code wgh_start} is longer than 160 ticks).
 */
@OnlyIn(Dist.CLIENT)
public final class FireDoorClientSounds {
	private static LoopingDoorSound moveLoop;
	private static LoopingDoorSound alarmLoop;
	private static MultiblockControllerBlockEntity active;

	private FireDoorClientSounds() {
	}
	...
	public static void tick(MultiblockControllerBlockEntity be) {
		...
		moveLoop = ensurePlaying(moveLoop, ModSounds.WGH_START.get(), at, 2.0F);
		alarmLoop = ensurePlaying(alarmLoop, ModSounds.DOOR_ALARM6.get(), at, 2.0F);
	}
	...
	private static void stopAll() {
		...
		if (alarmLoop != null) {
			mc.getSoundManager().stop(alarmLoop);
			alarmLoop = null;
		}
		active = null;
	}
