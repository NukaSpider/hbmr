...
/**
 * Loops {@code wgh_big_start} while a silo hatch is moving (matches 1.7.10 open/close loop).
 */
@OnlyIn(Dist.CLIENT)
public final class SiloHatchClientSounds {
	private static LoopingDoorSound moveLoop;
	private static MultiblockControllerBlockEntity active;

	private SiloHatchClientSounds() {
	}
	...
	public static void tick(MultiblockControllerBlockEntity be) {
		...
		moveLoop = ensurePlaying(moveLoop, ModSounds.WGH_BIG_START.get(), at, 2.0F);
	}
